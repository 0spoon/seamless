// demoseed populates a THROWAWAY Seamless data dir with a fictional six-week
// history for console screenshots. Run it against a fresh dir while no daemon
// holds the DB (single-writer SQLite):
//
...
func main() {
	...
	for _, sub := range []string{"memory", "notes"} {
		if err := os.MkdirAll(filepath.Join(*dataDir, sub), 0o755); err != nil {
			log.Fatalf("demoseed: mkdir: %v", err)
		}
	}
	db, err := store.Open(filepath.Join(*dataDir, "seam.db"))
	if err != nil {
		log.Fatalf("demoseed: open db: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("demoseed: files manager: %v", err)
	}
