...
# myapp scaffold (marked by .git/seamless-scene-fixture -- see the note below on
# why the marker lives inside .git/ and not the working tree).
if [[ -e "$dir" && ! -e "$dir/.git/seamless-scene-fixture" ]]; then
...
cat >"$dir/main.go" <<'EOF'
// myapp is a tiny web service. It signs users in with a rotating refresh-token
// scheme and serves a small HTML dashboard.
package main
...
Run `go run .` to start it on :8080.
EOF

cd "$dir"
git init -q
# The scaffold marker lives INSIDE .git/ (never the working tree, never a commit)
