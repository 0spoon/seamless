...
Seamless's Claude Code hooks are installed on this machine (`seamlessd install-hooks`): every session starts with an auto-injected `<seam-briefing>` scoped to the repo's project (your repo maps itself to a project automatically on its first session -- derived from the repo root; you never run a setup command, and `seamlessd map-repo` exists only to override the derived slug), and prompts that match stored memories get a `<seam-recall>` injection. Read what was injected before re-searching; use `mcp__seamless__recall` for on-demand retrieval.
...
- Use `mcp__seamless__memory_write` / `mcp__seamless__memory_read` for knowledge that should survive beyond this conversation (architectural decisions, debugging insights, gotchas, user preferences about their notes). `memory_write` requires `name`, `kind`, `description`, and `body`; trust the tool's schema for the field list, not this prose. `kind` is Seamless's own enum — `constraint`, `runbook`, `protocol`, `gotcha`, `decision`, `refuted`, `reference`, `stage` — and is NOT the file-based memory `type` vocabulary (`user`/`feedback`/`project`/`reference`): only `reference` overlaps, and `project` is a scope, never a kind. Scope is inferred from your bound/ambient session, so after `session_start` (or in a mapped repo with one unambiguous ambient session) you rarely pass `project:`; with no session it is ambiguous, so pass `project:` explicitly — or `project: global` for a deliberate cross-project memory. A durable create with no session and no `project:` is rejected rather than silently going global. The body param accepts `body`, `content`, or `text` interchangeably; `description` is one line, written for a future agent deciding whether to read it.
