...
- Use `mcp__seamless__memory_write` / `mcp__seamless__memory_read` for knowledge that should survive beyond this conversation (architectural decisions, debugging insights, gotchas, user preferences about their notes). Always pass `project:` (the repo's project slug, shown in the injected briefing) for project-specific knowledge — omitting it creates a global memory that will not appear in project-scoped briefings. `description` (one line, written for a future agent deciding whether to read it) is required.
