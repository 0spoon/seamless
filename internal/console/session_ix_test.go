...
func TestSessionDetail_InteractionsSurface(t *testing.T) {
	...
	require.Contains(t, body, `class="ptl-track"`, "renders the proportional wall-clock strip")
	require.NotContains(t, body, `class="ix-volume`, "the uniform-slice histogram is gone")
	...
	require.Contains(t, body, `data-ptl-anchor="`, "a block carries the trace row it jumps to")
}
