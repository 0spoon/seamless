-- New gardener proposal kind 'tool_error': agents keep hitting the same error
-- on a tool call (is_error tool.call events) or hook stage (hook.error events);
-- apply creates an open task to fix the recurring error.
--
...
CREATE TABLE gardener_proposals_new (
    id          TEXT PRIMARY KEY,
    kind        TEXT NOT NULL CHECK (kind IN ('merge','archive','digest','consolidate','reproject','split','abandon_plan','memory_wanted','tool_error')),
