-- New gardener proposal kind 'memory_wanted': agents repeatedly searched for
-- knowledge that does not exist (recurring zero-hit recall queries); apply
-- creates an open task to write the missing memory.
--
...
CREATE TABLE gardener_proposals_new (
    id          TEXT PRIMARY KEY,
    kind        TEXT NOT NULL CHECK (kind IN ('merge','archive','digest','consolidate','reproject','split','abandon_plan','memory_wanted')),
