...
> How RRF merges keyword and vector search into one ranked list by rank, not score - and how Seamless runs both legs in a single SQLite file with no vector database.
...
Seamless's [recall](https://thereisnospoon.org/docs/concepts/recall/) - the single search entry point
...
[utility nudge](https://thereisnospoon.org/docs/concepts/recall/#the-utility-nudge) capped at +10%. Both are
...
injection, explicit search), see [recall](https://thereisnospoon.org/docs/concepts/recall/); for what the
store itself looks like, see [memory & notes](https://thereisnospoon.org/docs/concepts/memory/).
