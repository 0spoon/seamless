...
> The coordination primitive that lets parallel AI agents share one task queue - an atomic claim with an expiry, so two agents never work the same task and a crashed agent never strands one.
...
[Codex CLI](https://thereisnospoon.org/docs/codex-cli/) agent share one queue - and they persist across
...
See it happen: [two agents race for the same plan step](https://thereisnospoon.org/scenarios/task-collision/),
...
[tasks & plans](https://thereisnospoon.org/docs/concepts/tasks-and-plans/).
