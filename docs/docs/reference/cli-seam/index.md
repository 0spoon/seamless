...
[Configuration](https://thereisnospoon.org/docs/reference/configuration/), so if the key is wrong or the daemon
...
The queue side of the CLI. See [Tasks](https://thereisnospoon.org/docs/reference/mcp/tasks/) for what ready,
...
2. **mcp_tools** - `tools/list` returns the tool count this CLI was built to
   ...
   feature](https://thereisnospoon.org/docs/reference/console/#optional-features) that is switched off (the
