...
`all` never fails on Linux over it). See [Codex local setup](https://thereisnospoon.org/docs/codex-cli/) for
the shared app/CLI/IDE profile and Codex's trust gate, and
[Claude app chat setup](https://thereisnospoon.org/docs/claude-app/) for what the chat surface does and does
not get.
